...
bool TMyProperty::InitDataSrc()
   ...
   TCnctDlg dlg( NULL );
			...
			CString sCnct;
			if( dlg.m_iSecurity )
			  sCnct.Format( _T("Provider=SQLOLEDB.1;Persist Security Info=False;User ID=%s;Initial Catalog=%s;Data Source=%s"), 
					  (LPCTSTR)dlg.m_csUser, (LPCTSTR)dlg.m_cs_DBName, (LPCTSTR)dlg.m_csServer );
			else
				 sCnct.Format( _T("Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=%s;Data Source=%s"), 
					  (LPCTSTR)dlg.m_cs_DBName, (LPCTSTR)dlg.m_csServer );

			MyCOMChk( m_pConnection->Open( (LPCTSTR)sCnct,	"", 
