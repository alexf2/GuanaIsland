...
class COXStaticText : public CStatic
{
// Data members -------------------------------------------------------------
public:
	CBitmap *m_BMP;
...
public:
	...
	BOOL SetText(LPCTSTR psText, BOOL bPrepareNow = FALSE);
	...
	//													specified, the application can only modify the world-to-device
	//													transform by calling functions that set window and viewport
	//													extents and origins, but not by using SetWorldTransform or
	...
	//													world transformations. This value must be specified if the
	//													application will set or modify the world transformation for
	//													the specified device context. In this mode all graphics,
	//													including text output, fully comforms to the world-to-device
	//													transformation specified in the device context.
	//													Windows 95: The GM_ADVANCED value is not supported. When
	//													playing enhanced metafiles, Windows 95 does its best to ensure
	//													that enhanced metafiles on Windows 95 look the same as they do
	//													on Windows NT. To accomplish this, Windows 95 may simulate
	//													GM_ADVANCED mode when playing specific enhanced metafile records.
	//					bPrepareNow = TRUE if internal image has to be rebuild.  If more than one
	//								  feature of the COXStaticText control is set, the last
	//								  set has to have this parameter set to TRUE, to have the image build
	// --- Out :
